...
async def save_group(bot, message):
    ...
    if temp.ME in r_j_check:
        ...
        if message.chat.id in temp.BANNED_CHATS:
            # THOSE WHO WERE AFRAID OF THE LAW OF THE INDIAN HIGH COURT AND DELETED THE CODE IN THE REPO 
            buttons = [[
        ...
        await message.reply_text(
            text=f"<b>›› THANKS FOR ADDING ME INTO YOUR GROUP.\n›› PLEASE  𝙳𝙾𝙽'𝚃 𝙵𝙾𝚁𝙶𝙴𝚃 𝚃𝙾 𝙼𝙰𝙺𝙴 𝙼𝙴 𝙰𝙳𝙼𝙸𝙽.\n›› IF YOU ARE HAVING 𝙰𝙽𝚈 𝙳𝙾𝚄𝙱𝚃𝚂 𝙰𝙱𝙾𝚄𝚃 𝚄𝚂𝙸𝙽𝙶 𝙼𝙴 𝙲𝙻𝙸𝙲𝙺 𝙱𝙴𝙻𝙾𝚆 𝙱𝚄𝚃𝚃𝙾𝙽..⚡⚡.</b>",
            reply_markup=reply_markup)
    else:
...
@Client.on_message(filters.command('leaveg') & filters.user(ADMINS))
...
@Client.on_message(filters.command('gdisable') & filters.user(ADMINS))
...
@Client.on_message(filters.command('genable') & filters.user(ADMINS))
...
@Client.on_message(filters.command('clink') & filters.user(ADMINS))
async def gen_invite(bot, message):
    ...
    try:
        ...
        return await message.reply("Invite Link Generation Failed Because I Am Not Having Sufficient Rights To Generate The Invite Link ")
    except Exception as e:
...
async def ban_a_user(bot, message):
     # THOSE WHO MADE FUN OF TE BANANA TREE GOT SCARED AND RAN AWAY
    if len(message.command) == 1:
        return await message.reply('Give me an user id / username')
    r = message.text.split(None)
    if len(r) > 2:
        reason = message.text.split(None, 2)[2]
        chat = message.text.split(None, 2)[1]

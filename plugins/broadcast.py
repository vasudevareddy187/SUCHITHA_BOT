...
# THOSE WHO MADE FUN OF THE VAZHA TREE##fi
async def verupikkals(bot, message):
    ...
    async for user in users:
        ...
        formatted_msg = b_msg.text.replace('{first_name}', first_name).replace('{last_name}', last_name)
        pti, sh = await broadcast_messages(int(user['id']), Message(text=formatted_msg))
        if pti:
            success += 1
        elif pti == False:
            if sh == "Blocked":
